...
const (
	DownloadTypeAudio DownloadType = "audio"
...
// DownloadInstagram downloads content from Instagram using yt-dlp
func DownloadInstagram(url string, downloadType DownloadType) (*DownloadResult, error) {
	tempDir, err := os.MkdirTemp("", "insta-download-*")
	if err != nil {
	...
	return &DownloadResult{
		FilePath:    files[0],
		Filename:    fmt.Sprintf("instagram_%s.%s", downloadType, fileExt),
		ContentType: contentType,
	}, nil
}

// CleanupDownload removes the temp directory after download is complete
func CleanupDownload(filePath string) {
