...
// DownloadYouTube downloads content from YouTube using yt-dlp
func DownloadYouTube(url string, downloadType DownloadType) (*DownloadResult, error) {
	tempDir, err := os.MkdirTemp("", "youtube-download-*")
	if err != nil {
	...
	return &DownloadResult{
		FilePath:    files[0],
		Filename:    fmt.Sprintf("youtube_%s.%s", downloadType, fileExt),
